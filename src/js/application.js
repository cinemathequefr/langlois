(function () {
  ...
  app.controller = function () {
    ...
    if (type === "point") {
      if (id) {
        ...
        if (!point.isComplete) {
          // If the point is not complete, we fetch the extra data before rendering
          $.when(app.fetchData("data/points/" + id + ".json")).then(function (
            data
          ) {
  ...
  app.init = function (data) {
    app.points = _.sortBy(data, function (i) {
      return i.pos;
    }); // Data sorted by pos
    ...
    app.points.get = function (id) {
      return _.find(this, function (pt) {
        return pt.id === id;
      });
    };
    ...
    app.$pointContainer = $("<div class='point-container'></div>").appendTo(
      ".container"
    );

    _.each(app.points, function (point, i) {
      // Populate timeline with points
    ...
    $(window)
      .on("debouncedresize.main", app.quadrant, function (e) {
        var quadrant = e.data;
        quadrant.render();
        quadrant.scrollTo({ pos: quadrant.currentPos });

        app.$.container.css({
          backgroundImage: "url(css/images/langlois.jpg)",
        });
  ...
  app.renderIndex = function () {
    app.state.transitionOut(function () {
      app.$pointContainer
        .hide()
        .html(
          "<div class='center'><p>La vie d'Henri Langlois (1914-1977) se confond largement avec l'existence de la Cinémathèque française, institution qu'il a créée en 1936 avec Georges Franju, Paul-Auguste Harlé et Jean Mitry.</p><p>À travers plus de 120 dates, importantes ou anecdotiques, parcourez en tous sens l'histoire mouvementée de l'homme qui a poursuivi inlassablement la mission qu'il s'était fixé : sauver les films de la destruction et de l'oubli, les montrer.</p><p><a class='enter' href='#!/point/113'>Entrer</a></p></div>"
        );
  ...
  app.renderPoint = function (point) {
    app.state.transitionOut(function () {
      ...
      app.$pointContainer
        .html(Mustache.render(app.templates.point, point))
        .imagesLoaded(function () {
          ...
          $(".media > img").on("click", function () {
            app.dz = new DeepZoom({
              $el: $(".overlay"),
              url:
                "http://cf.pasoliniroma.com/static/langlois/dz/" + point.m.id,
              width: point.m.width,
  ...
  app.pointResize = function (point) {
    var $p = app.$pointContainer.css({
      visibility: "hidden",
      display: "block",
    }); // For all computations, elements must be hidden but dimensioned
  ...
  // app.state
  app.state = (function () {
    var current, old, get, getOld, navigate, save, transitionOut;
    ...
    transitionOut = function (onAfter) {
      ...
      if (!_.isUndefined(app.dz) && app.dz.isOpen) {
        // Close deepZoom overlay if necessary
        app.dz.close();
      }
  ...
  $(function () {
    $.when(app.fetchData("data/index.json")).then(app.init, app.fail);
  });
})();
